...
ethers.js
Database
MongoDB (stores public keys + DIDs, no passwords)
...
No password exists
Breach exposes all users
Breach exposes nothing useful
Platform owns your identity
...
Demo Flow (2 minutes)
Create an identity → show keypair generation, DID, identity card
Show the transaction on Polygonscan — identity is on-chain
Log into a demo service using signature-based auth — no password prompt
Attempt to log in with a wrong key — show it fails
Show MongoDB — only public keys, nothing sensitive

Impact
Applicable to any platform that currently uses password-based auth — social media, fintech, healthcare. VaultID's architecture means even if the server is fully compromised, there are zero user credentials to steal.
